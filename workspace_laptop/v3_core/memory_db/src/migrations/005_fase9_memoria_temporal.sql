...
INSERT INTO memory_policy (id, retention_policy) VALUES (1, '1d') ON CONFLICT (id) DO NOTHING;
...
CREATE INDEX IF NOT EXISTS idx_customer_profiles_active_expiry ON customer_profiles(status, expires_at);
...
CREATE INDEX IF NOT EXISTS idx_customer_memory_profile_status ON customer_memory_entries(profile_id, status, memory_type, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_customer_memory_expiry ON customer_memory_entries(status, expires_at);
...
CREATE INDEX IF NOT EXISTS idx_memory_audit_profile_time ON memory_audit_events(profile_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_memory_audit_event_time ON memory_audit_events(event, created_at DESC);
